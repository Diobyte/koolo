...
func StartGame(username string, password string, authmethod string, authToken string, realm string, arguments string, useCustomSettings bool) (uint32, win.HWND, error) {
	...
	if useCustomSettings {
		...
		// If there is no real mod, let's create a fake mod so we can store our own config
...
// getD2RPIDs returns the set of all currently running D2R.exe process IDs.
func getD2RPIDs() (map[uint32]struct{}, error) {
...
func startGameSteam(arguments string, useCustomSettings bool) (uint32, win.HWND, error) {
	...
	for attempt := 0; attempt < maxGPURetries; attempt++ {
		...
		if err != nil {
			return 0, 0, fmt.Errorf("failed to snapshot existing D2R PIDs: %w", err)
		}

		// Build the Steam launch URI. Use steam://run/ which properly forwards CLI
		// arguments to the game executable via the "//" separator.
		launchURI := "steam://run/" + steamAppID
		if len(extraArgs) > 0 {
			launchURI += "//" + strings.Join(extraArgs, " ")
		}

		slog.Info("Launching D2R via Steam", slog.String("uri", launchURI), slog.Int("attempt", attempt+1))

		cmd := exec.Command("cmd", "/c", "start", "", launchURI)
		if err := cmd.Start(); err != nil {
			return 0, 0, fmt.Errorf("failed to open Steam URI: %w", err)
		}
		...
		for time.Now().Before(deadline) {
			...
			for pid := range currentPIDs {
				if _, existed := existingPIDs[pid]; !existed {
					newPID = pid
					break
				}
			}
		...
		slog.Info("Found new D2R process", slog.Uint64("pid", uint64(newPID)))

		// Now find the HWND for this PID.
		var foundHwnd windows.HWND
		cb := syscall.NewCallback(func(hwnd windows.HWND, lParam uintptr) uintptr {
			var pid uint32
			windows.GetWindowThreadProcessId(hwnd, &pid)
		...
		hwndDeadline := time.Now().Add(30 * time.Second)
		for time.Now().Before(hwndDeadline) {
			windows.EnumWindows(cb, unsafe.Pointer(&newPID))
			if foundHwnd != 0 {
				// Small delay and re-read to be sure we captured the right hwnd.
				time.Sleep(time.Second)
				windows.EnumWindows(cb, unsafe.Pointer(&newPID))
				break
			}
		...
		if foundHwnd == 0 {
			return 0, 0, errors.New("found D2R process but could not locate its window")
		}
		...
		// Release client handles to allow subsequent instances.
		if err := KillAllClientHandles(); err != nil {
			return 0, 0, err
		}
